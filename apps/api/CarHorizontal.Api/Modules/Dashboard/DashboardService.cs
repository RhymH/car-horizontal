...
public class DashboardService : IDashboardService
{
    ...
    public async Task<DashboardOverviewResponseDto> GetOverviewAsync(CancellationToken ct = default)
    {
        ...
        var loyaltyTrend = BuildLoyaltyTrendStub(startOfMonth);
        ...
        CancellationToken ct)
    {
        ...
        var atRisk = await _db.Customers.AsNoTracking()
            .Where(c => c.Status == CustomerStatus.Active)
            .Where(c => !_db.CustomerInteractions
                .Any(i => i.CustomerId == c.Id && i.OccurredAt >= staleSince))
            .Where(c => !_db.MaintenanceRecords
                .Any(m => _db.Vehicles.Any(v => v.Id == m.VehicleId && v.CustomerId == c.Id)
                    && m.PerformedAt >= staleSince))
            .CountAsync(ct);
    ...
    private static List<DashboardChartPointDto> BuildLoyaltyTrendStub(DateTime startOfCurrentMonth)
    {
        var points = new List<DashboardChartPointDto>(12);
        for (var i = 11; i >= 0; i--)
        {
            var d = startOfCurrentMonth.AddMonths(-i);
            points.Add(new DashboardChartPointDto
            {
                Date = DateTime.SpecifyKind(d, DateTimeKind.Utc),
                Label = d.ToString("MMM yy", new CultureInfo("fr-FR")),
                Value = 0
            });
        }
        return points;
